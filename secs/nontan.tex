...
\begin{frame}[t]
  ...
  \end{prop}}
  \only<2-5>{\textit{Traccia della dimostrazione:} senza perdita di generalità $\sigma=1$.}\pause \only<3-5>{ Possiamo scrivere $S(1,\alpha)=\left\{z \in \mathbb{D} \mid |\mathfrak{Im}(z)|<(\tan{\alpha})\bigl(1-\mathfrak{Re}(z)\bigr)\right\}$.} \pause \only<4-5>{Se $z \in K(1,M)$, da
  $$M>\frac{|1-z|}{1-|z|}\ge \frac{|1-z|}{1-\mathfrak{Re}(z)}$$} \pause
  \only<5>{troviamo
  $$\frac{|\mathfrak{Im}(z)|}{1-\mathfrak{Re}(z)}<\sqrt{M^2-1}=\tan{\alpha};$$
  questo mostra la seconda inclusione.}
  \only<6-8>{\textit{Traccia della dimostrazione:} sia $\alpha'<\alpha$ e supponiamo per assurdo che per ogni $\epsilon>0$ esista $z \in S(1,\alpha')\cap B(1,\epsilon)$ tale che $z \not\in K(1,M)$.} \pause \only<7-8>{ Si ha allora
  \begin{equation} \label<7->{star1}
    \dfrac{1-|z|}{|1-z|} \le \dfrac{1}{M} \text{ e } \frac{|\mathfrak{Im}(z)|}{1-\mathfrak{Re}(z)}<\tan{\alpha'}.
  \end{equation}} \pause
  \only<8>{Dalla seconda disuguaglianza in \eqref{star1} si ottiene
  \begin{equation} \label<8->{star2}
  ...
  \only<9-11>{\textit{Traccia della dimostrazione:} $\dfrac{1-|z|}{1-\mathfrak{Re}(z)}<\dfrac{M'}{M}<1$.} \pause \only<10-11>{ Tuttavia, ponendo $x=\mathfrak{Re}(z)$ e $y=\mathfrak{Im}(z)$ e riscrivendo la condizione $z \in S(1,\alpha')$ come $y/(1-x)<\tan{\alpha'}$,} \pause \only<11>{vediamo facilmente che
...
\begin{frame}[t]
  ...
  \pause \only<4->{Fissato $\epsilon>0$, esiste $\delta>0$ tale che $|f(w)-w| \le \epsilon|1-w|^3$ per $w \in C(z)$, con $z \in K(1,M)\cap B(1,\delta/2)$. Si ottiene
  $$|I(z)| \le \frac{\epsilon}{r(z)}\max_{w \in C(z)}|1-w|^3.$$}
  \pause \only<5->{Da considerazioni geometriche si ha $|I(z)| \le \epsilon |z-1|^2\bigl(1+\csc(\beta-\alpha)\bigr)^3$, da cui $f'(z)=1+o\bigl((z-1)^2\bigr)$. Inoltre, dalle ipotesi
