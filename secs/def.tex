\begin{frame}[t]
  ...
  \only<1>{Setting: fissiamo $\Omega \subseteq \mathbb{C}^n, n \ge 2$ un dominio limitato con bordo $C^2$, cioè esiste $\rho \in C^2(\mathbb{C}^n)$ tale che $\Omega=\{\rho(z)<0\}$ e $\diff\rho\not=0$ in ogni punto di $\partial\Omega$.
  ...
  \end{defn}}
  \only<3>{Nel seguito, lavoriamo sempre sotto l'ipotesi che $\Omega$ sia limitato e strettamente pseudoconvesso.}
  \only<4>{
  \begin{defn}
    Sia $\mathbb{D}$ il disco unitario in $\mathbb{C}$, data $f:\mathbb{D} \longrightarrow \mathbb{C}^n$ olomorfa indichiamo con $Df(z)$ il differenziale di $f$ in $z \in \mathbb{D}$. La \textit{metrica di Kobayashi} su $\Omega$ limitato è
...
\begin{frame}
  ...
  \pause
   ...
   Zimmer, 2022: i domini convessi limitati (non necessariamente con bordo liscio) sono iperbolici con la distanza indotta dalla metrica di Kähler.
  \pause

  Seguono risultati, analoghi a quelli visti, di estensione al bordo e dinamica olomorfa.\pause Valgono anche delle stime subellittiche per le soluzioni del problema $\bar{\partial}$-Neumann, già estensivamente studiate per i domini strettamente pseudoconvessi (quindi con bordo liscio).
\end{frame}
