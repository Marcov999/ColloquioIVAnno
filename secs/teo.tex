...
\begin{frame}[t]
  ...
  \begin{thm}
    ...
    è una distanza su $\text{Con}(Z)$ che lo rende uno spazio iperbolico, il cui bordo può essere identificato con $Z$. Inoltre, per ogni $x,y \in Z$ si ha
  ...
  \only<10->{
  La disuguaglianza finale non è difficile, ma richiede un po' di passaggi che non vedremo.}\only<11>{ Mettendo su $\text{Con}(Z)\cup\partial_G\text{Con}(Z)$ un'opportuna topologia, dalla disuguaglianza segue anche che $Z$ e $\partial_G\text{Con}(Z)$ sono identificati come spazi topologici. \qed
  }
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{thm}
    Allora esiste $C \ge 0$ tale che per ogni $x, y \in \Omega$ vale
    \begin{equation} \label{stimadistanzafinsler}
      g(x,y)-C \le d_F(x,y) \le g(x,y)+C. \tag{2}
    \end{equation}
  \end{thm}
  }
  \only<6->{
