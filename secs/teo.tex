...
\begin{frame}[t]
  ...
  \only<1->{
  Sia $N_{\epsilon}(\partial\Omega)=\{x \in \mathbb{C}^n \mid \delta(x)<\epsilon\}$. Si può dimostrare che esiste $\epsilon>0$ tale che per ogni $x \in N_{\epsilon}(\partial\Omega)$ esiste un unico $\pi(x) \in \partial \Omega$ tale che $|x-\pi(x)|=\delta(x)$; inoltre, possiamo chiedere altre proprietà di utilità tecnica.
  }
  \only<2->{

  \%Da fare: qua credo che andrebbe messo il disegno

  }
  \only<3>{
  Dati $x,y \in N_{\epsilon}(\partial\Omega)\cap\Omega$, poniamo
